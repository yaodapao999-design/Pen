...
// EDITS BEGIN
void AdditionalLightsToonAttenuation_float(float3 WorldPosition, float3 WorldNormal, half4 Shadowmask,
						float PointLightBands, float SpotLightBands,
						out float3 Diffuse, out float Attenuation)
{
    float3 diffuseColor = 0;
    float attenSum = 0;
	...
	for (uint lightIndex = 0; lightIndex < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, MAX_VISIBLE_LIGHTS); lightIndex++) {
...
#endif
		{
			if (PointLightBands <= 1 && SpotLightBands <= 1){
				// Solid colour lights
				attenSum += light.distanceAttenuation * light.shadowAttenuation;
				diffuseColor += light.color * step(0.0001, light.distanceAttenuation * light.shadowAttenuation);
			}else{
				attenSum += light.distanceAttenuation * light.shadowAttenuation;
				// Multiple bands
